...
def print_models_table(
    data: Dict[str, Any], format_dates: bool = False, colorize: bool = True
) -> None:
    """
...
def sort_models_data(
    data: Dict[str, Any], sort_field: str, descending: bool = False
) -> Dict[str, Any]:
    """
...
def print_models_json(
    data: Dict[str, Any], pretty: bool = True, format_dates: bool = False, colorize: bool = True
) -> None:
    """
...
def cmd_list_models(args: argparse.Namespace) -> int:
    ...
    if not api_key:
        print(
            "Error: OpenAI API key not found. Please set the _OPENAI_API_KEY environment variable, "
            "provide it with --key, or use 'bu-openai.py configure' to set up.",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        print(
            f"Error: API request failed (Status code: {status_code})\n{error_msg}", file=sys.stderr
        )
        return 1
...
def setup_parsers() -> argparse.ArgumentParser:
    ...
    """
    # Main parser
    main_parser = argparse.ArgumentParser(description="Command-line interface for OpenAI API")

    # Add global arguments
    main_parser.add_argument(
        "-k", "--key", help="OpenAI API key (overrides env var and config file)"
    )

    # Add version argument
    main_parser.add_argument("-v", "--version", action="version", version="bu-openai 0.1.0")
    ...
    models_parser.add_argument(
        "-c", "--compact", action="store_true", help="Output compact JSON without pretty printing"
    )
    models_parser.add_argument("-f", "--filter", help="Filter models by name (case-insensitive)")
    models_parser.add_argument(
        "-d",
        "--pretty-dates",
        action="store_true",
    ...
    models_parser.add_argument("--no-color", action="store_true", help="Disable colorized output")
    models_parser.add_argument(
        "-t", "--table", action="store_true", help="Display results in a tabular format using Rich"
    )
    models_parser.add_argument(
        "--sort",
    ...
    models_parser.add_argument(
        "--asc", action="store_true", help="Sort in ascending order (default when --sort is used)"
    )
    models_parser.add_argument("--dsc", action="store_true", help="Sort in descending order")
